...
このプロジェクトは、AWS上にWebアプリケーション用インフラを構築することを題材としたポートフォリオである。

主な目的は、インフラ・クラウド技術の学習と実践であり、アプリケーション開発自体は主目的ではありません。段階的にAWS、Terraform、Docker、CI/CD、Observabilityなどの技術を導入し、最終的には本番環境に近い構成を目指す。
